...
public class ExchangeRateDataApiProviderTest
{
    [Theory]
    ...
    public async Task GetCurrentRate_ReturnsRate(string baseCurrency, string targetCurrency, decimal expectedRate)
    {
        ...
        // json from ExchangeRateDataApi documentation or live request
        mockHttp.When($"https://api.apilayer.com/exchangerates_data/latest?base={baseCurrency}&symbols={targetCurrency}")
        ...
        NullLogger<ExchangeRateDataApiProvider> logger = new();
        ...
        IRateProvider rateProvider = new ExchangeRateDataApiProvider(logger, httpClient, mockRateProviderKeyOption.Object);
    ...
    [Fact]
    public async Task GetCurrentRate_ExchangeRateDataApiFails_ThrowsRateProviderException()
    {
        // arrange            
    ...
    public async Task GetCurrentRate_ReceiveErrorJsonResponse_ThrowsRateProviderException()
    {
        ...
        mockHttp.When($"https://api.apilayer.com/exchangerates_data/latest?base=EUR&symbols=AUD")
               .Respond("application/json", @$"{{
                        ""success"": false,
                        ""error"": {{
                            ""code"": 201,
    ...
    public async Task GetCurrentRate_ReceiveUnexpectedJson_ThrowsException()
    {
        ...
        Assert.StartsWith("Unexpected error, Exchange Rate Data Api response " +
    ...
    public void GetCurrentRate_NoApiKeys_ThrowArgumentException()
    {
        ...
        var exception = Assert.Throws<ArgumentException>(
            () => new ExchangeRateDataApiProvider(logger, httpClient, mockRateProviderKeyOption.Object));

        Assert.Equal("Missing ExchangeRateDataApi api key", exception.Message);
    }
}